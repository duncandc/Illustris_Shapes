...
def main():
    ...
    fields = ['SubhaloGrNr', 'SubhaloMassInRadType', 'SubhaloPos', 'SubhaloVel']

    galaxy_table = loadSubhalos(basePath, snapNum, fields=fields)
    
    # FoF host halo ID
    host_halo_id = galaxy_table['SubhaloGrNr'][halo_ids]
    
    x = galaxy_table['SubhaloPos'][:,0][halo_ids]
    y = galaxy_table['SubhaloPos'][:,1][halo_ids]
    z = galaxy_table['SubhaloPos'][:,2][halo_ids]

    vx = galaxy_table['SubhaloVel'][:,0][halo_ids]
    vy = galaxy_table['SubhaloVel'][:,1][halo_ids]
    vz = galaxy_table['SubhaloVel'][:,2][halo_ids]

    mstar_in_twice_halfrad = galaxy_table['SubhaloMassInRadType'][:,4][halo_ids]
    mstar_in_twice_halfrad = mstar_in_twice_halfrad*10**10

    mstar_all = galaxy_table['SubhaloMassType'][:,4][halo_ids]
    ...
    central_id = host_halo_table['GroupFirstSub'][host_halo_id]

    # host halo properties
    host_halo_mass_200m = host_halo_table['Group_M_Mean200'][host_halo_id]
    host_halo_radius_200m = host_halo_table['Group_R_Mean200'][host_halo_id]
    host_halo_fof_mass = host_halo_table['GroupMass'][host_halo_id]
    ...
    ascii.write([gal_ids, host_halo_ids, central_id,
                ...
                fpath+fname,
                ['gal_id', 'host_halo_id', 'central_id',
                 'x', 'y', 'z',
                 'vx', 'vy', 'vz',
